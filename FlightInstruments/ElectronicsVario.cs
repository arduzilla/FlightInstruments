...
namespace FlightInstruments
{
    ...
    public partial class EVarioControl : InstrumentBase
    {
        private double nettoVario;
        private double integrator;
        public EVarioControl()
        {
            DoubleBuffered = true;
        ...
        {
            DrawEVario(e.Graphics);
        }

        public double NettoVario
        {
            get { return nettoVario; }
            set
            {
        ...
        private void DrawEVario(Graphics g)
        {
...
//
        protected override void OnUDPDataReceived(string udpData)
        {
            try
            {
                ...
                if (jsonObject.TryGetValue("evario", out JToken? outToken) && outToken != null && outToken.Type == JTokenType.Float)
                {
                    double val = outToken.ToObject<double>() * 3.28084;
                    TargetValue = val; // Update the property based on JSON data
                }
                if (jsonObject.TryGetValue("nettovario", out outToken) && outToken != null && outToken.Type == JTokenType.Float)
                {
                    double val = outToken.ToObject<double>() * 3.28084;
                    NettoVario = val; // Update the property based on JSON data
                }
                if (jsonObject.TryGetValue("integrator", out outToken) && outToken != null && outToken.Type == JTokenType.Float)
                {
                    double val = outToken.ToObject<double>() * 3.28084;
                    Integrator = val; // Update the property based on JSON data
                }
